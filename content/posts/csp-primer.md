...
A lot of problems can be described as a constraint satisfaction problem, abbreviated as CSP.
...
Usually the domain is the same for all of the variables, but it is also possible that the variables have different domains.
...
The type of domain that is used for the variables plays a major factor in the different approaches that are used to solve the problem. If a domain contains a zero and a one, it is called a _Boolean domain_. 
...
This does not seem that difficult, but there is a catch, it should be solved by only using three colors.
...
The next thing that we need to do is define the domain for each variable. 
Because we can only use three colors—red, green, and blue—all the variables have the same domain.
...
If we look at the map and create a node for each region, if we then connect all the adjacent regions the result is the following graph.
...
This type of graph is called a _constraint graph_, and it visualizes how the variables and the constraints are related to one another.
...
We have now defined the map coloring problem as a constraint satisfaction problem, and are ready to solve it.
...
A _solution_ is a tuple with a configuration that satisfies all of the constraints. 
...
It is also possible to have a _partial configuration_, which is a tuple where not all of the variables have been assigned.
...
The easiest way, and the most inefficient method, to find a solution to the map coloring problem is to generate all the of the configurations and see which of those configurations satisfies all of these constraints. The map coloring problem only has 2187 different configurations so checking all of them easily done on a computer. This problem only has seven variables, but the search space grows with $3^n$, which is exponentially.
To generalize this, if we have equal domains of size $d$ and a number of variables $n$, the size of the search space if $d^n$. 
This quickly becomes a hopeless approach.
So what can we do about this?

### Search trees

The search space can be visualized with a _search tree_. The illustration below shows the search tree of the map coloring problem. 
...
The edges are the possible assignments of that variable. 
At the end of the edge is the next variable, with the possible assignments of that variable and so on. The illustration becomes too big to draw pretty quickly, so usually only the assigned path is drawn. 
This type of visualization is very helpful, because it provides a visual way of thinking about constraint satisfaction problems.

### Backtracking

The key is to check the partial configurations that are generated by the brute force algorithm. Once we find that a partial configuration is violating a constraint, we can immediately stop with that partial solution and try the next one. 
As an example, suppose red is assigned to $WA$, then a constraint is violated if red is also assigned to $SA$. Therefore we can stop immediately after finding out that it is not possible to assign red to $SA$, which results in pruning all of the configuration that are under that branch of the search tree. 

This is already a huge improvement over the brute force approach, but even this method will fail to find solutions in a timely manner for bigger problems. In later posts we will look into other techniques that give massive performance improvements to the solving bigger problems.
...
To make it easy to see if we have satisfied all the constraints, we will add the constraint graph to the program.
...
The next thing we will do is use the `EdgeTo` method to define the constraint graph of the problem with an adjacency list.
...
Now that we know how all of the variables are connected to one and another, we will implement a method that checks if all the constraints are satisfied. This means that we need to check if all the regions that are adjacent don't have the same color. A constraint is only checked if it is in scope, or in other words, if all the variables in the constraint have been assigned.
...
The base case of the function should return if we have assigned all the variables. This happens when `k` is bigger than or equal to the number of variables. This leads to the following implementation.
...
If we run this algorithm, the solution is stored in the array `variables` after the program has finished. 
...
It is possible to find all of the solution to the map coloring problem with only a minor modification to the algorithm. On line 5, instead of returning `true` which results in stopping the search procedure, we need to save the solution and return `false`.
